...
def in_order_traversal(root=None):
    if not root:
        return []
    left_subtree = in_order_traversal(root.left)
    right_subtree = in_order_traversal(root.right)
    return left_subtree + [root.value] + right_subtree
    ...
    5. Return the left list values concatenated with the root value,
...
def post_order_traversal(root=None):
    if not root:
        return []
    left_sub = post_order_traversal(root.left)
    right_sub = post_order_traversal(root.right)
    return left_sub + right_sub + [root.value]
